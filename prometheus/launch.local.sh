...
echo -e "\e[1;37mdocker run -d --name=prom-nodeexporter --net=host --pid=host --rm prom/node-exporter\e[0m"
docker run -d --name=prom-nodeexporter --net="host" --pid="host" --rm prom/node-exporter

echo -e "\e[1;33mLaunching prometheus local\e[0m"
echo -e "\e[1;37mdocker run -d --name=prom-local --net=host -v $PWD/local.prometheus.yml:/etc/prometheus/prometheus.yml --rm prom/prometheus\e[0m"
...
echo -e "\e[1;37mdocker run -d --name=grafana --net=host --rm grafana/grafana\e[0m"
